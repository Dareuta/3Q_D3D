...
bool TutorialApp::InitScene()
{
	...
	// BasicVertexShader.hlsl이라는 파일 내부 main을 찾아서, vs_4_0 버전으로 읽어서 가져오는거임
	// vertexShaderBuffer여기에 담는거
	// 쉐이더 파일을 읽어오는거
	...
	//CreateInputLayout의 원형
	//	HRESULT CreateInputLayout(
	//		const D3D11_INPUT_ELEMENT_DESC * pInputElementDescs, // 입력 요소 배열
	//		UINT NumElements,									 // 배열 원소 개수
	//		const void* pShaderBytecodeWithInputSignature,		 // 셰이더 바이트코드 (VS 컴파일 결과)
	//		SIZE_T BytecodeLength,								 // 바이트코드 크기
	//		ID3D11InputLayout * *ppInputLayout                   // 결과: InputLayout 객체
	//	);

	// 해당 코드는, 버텍스 셰이더가 어떤 입력을 기대하고 있는지 확인하는 맵핑과정임
	...
	//		SIZE_T BytecodeLength,                // 바이트코드 크기
	//		ID3D11ClassLinkage * pClassLinkage,    // 셰이더 클래스 동적 연결(일반적으로 NULL)
	//		ID3D11VertexShader * *ppVertexShader   // 결과: 생성된 VertexShader 객체
	//	);


	SAFE_RELEASE(vertexShaderBuffer);	// 버퍼 해제.
	// 이전과 마찬가지로, 원본에 대해 참조가 걸리는거 해제하는거임,
	// 즉, 소유권 이전
	...
	WORD indices[] =
	{
		0, 1, 2,
		2, 1, 3
	}; // 그 유명한 인덱스 버퍼임, 각 번호가 정점 버퍼를 의미함

	m_nIndices = ARRAYSIZE(indices);	// 인덱스 개수 저장.

	D3D11_BUFFER_DESC ibDesc = {}; // 이거랑
	ibDesc.ByteWidth = sizeof(WORD) * ARRAYSIZE(indices);
	ibDesc.BindFlags = D3D11_BIND_INDEX_BUFFER;
	ibDesc.Usage = D3D11_USAGE_DEFAULT;

	D3D11_SUBRESOURCE_DATA ibData = {}; // 이거 만들어서
	ibData.pSysMem = indices; // 인덱스 버퍼 넣어줌
